...
def monitor_mqtt_broker(userID, CTName, port):
    ...
    porty = container_query['port']
    if CTCreds == 0:
        arg_cmd = "--port " + str(porty) + " --user " + CT_user + " --ip " + CT_ip + " --topic " + "#" + " --ctname " + CTName + " --cred " + str(CTCreds)
    else:
    ...
    ctn = client.containers.run("ekstrah/monitor-api:latest", arg_cmd,  detach=True)
    return ctn
...
def create_container(data, CTName=None):
    flag = 0
    while flag == 0:
        rand_port = random.randint(20000, 20400)
    ...
    if CTName is None:
        container = client.containers.run(image='eclipse-mosquitto:2.0.13', detach=True, ports=port_dict)
        CTName = container.name
    else:
        container = client.containers.run(image='eclipse-mosquitto:2.0.13', detach=True, ports=port_dict, volumes=vol, name=CTName)
        CTName = container.name
    if data['type'] == 1:
